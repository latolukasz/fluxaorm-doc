...
tagline: Code-generation-based Go ORM for MySQL, Redis, and ClickHouse with type-safe Providers, dirty tracking, and three-tier caching
...
- title: MySQL + ClickHouse + Redis Search
  details: Full MySQL support with type-safe queries, query-only ClickHouse integration for analytics, and Redis Search Engine for lightning-fast full-text and numeric searches. Async SQL operations via Redis Streams for non-blocking writes.
  footer: MIT Licensed | Copyright © 2024-present Łukasz Lato
---
